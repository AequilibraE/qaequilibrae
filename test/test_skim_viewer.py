...
def test_plot_with_joined_results(sf_project_with_assignment, qtbot, timeoutDetector, mocker, qgis_iface):
    lyr = sf_project_with_assignment.layers["nodes"][0]
    QgsProject.instance().addMapLayer(lyr)
    qgis_iface.setActiveLayer(lyr)
    ...
    dialog = SkimViewerDialog(sf_project_with_assignment)
