...
# On Linux and macOS, `packages` stays profile-local so QGIS can install dependencies on first load.
[arg('tag', pattern='3|4')]
[linux]
link tag="3" profile="default":
    set -euo pipefail; \
    plugin_dir="$HOME/.local/share/QGIS/QGIS{{ tag }}/profiles/{{ profile }}/python/plugins"; \
