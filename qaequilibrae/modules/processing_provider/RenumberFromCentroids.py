...
class RenumberFromCentroids(TranslatableAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        feedback.pushInfo(tr('Importing node layer from QGIS'))
        ...
        feedback.pushInfo(tr('Looking for existing nodes from AequilibraE project'))
        #Import QGIS layer as a panda dataframe
        ...
        feedback.pushInfo(tr('Comparing nodes from input centroid layer:'))
        ...
        feedback.pushInfo(tr('With existing nodes in AequilibraE project:'))
        ...
        for idx, zone in df.iterrows():
            ...
            if (len(matching.index) == 1):
                ...
                fail += 1
                feedback.pushInfo(tr(' Multiple nodes found for zone ')+ str(zone[parameters['node_id']])+tr(', unable to decide which node to use'))
        feedback.pushInfo(str(len(df))+tr(' zones found in input layer :'))
        if find>0:
            feedback.pushInfo(' •'+str(find)+tr(' zones found an existing matching node'))
        if create>0:
            feedback.pushInfo(' •'+str(create)+tr(' new nodes added for unmatched zones'))
        if fail>0:
            feedback.pushInfo(' •'+str(fail)+tr(' zones could not be processed'))
        feedback.pushInfo(' ')
