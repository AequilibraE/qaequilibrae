    ...
    def processAlgorithm(self, parameters, context, model_feedback):
        ...
        for idx, zone in df.iterrows():
            ...
            if (len(matching.index) == 1):
                ...
                fail += 1
                feedback.pushInfo(self.tr(' Multiple nodes found for zone ')+ str(zone[parameters['node_id']])+self.tr(', unable to decide which node to use'))
        feedback.pushInfo(str(len(df))+tr(' zones found in input layer :'))
        if find>0:
            feedback.pushInfo(' •'+str(find)+self.tr(' zones found an existing matching node'))
        if create>0:
            feedback.pushInfo(' •'+str(create)+self.tr(' new nodes added for unmatched zones'))
        if fail>0:
            feedback.pushInfo(' •'+str(fail)+self.tr(' zones could not be processed'))
        feedback.pushInfo(' ')
