...
class CreateScenariosDialog(QtWidgets.QDialog, FORM_CLASS):
    ...
    def run(self):
        ...
        if self.rdo_clone.isChecked():
            ...
            #     QgsMessageLog.logMessage(f"Changing scenario from {self.__init_scenario} to {self.cob_scenarios.currentText()}", "Messages", Qgis.Info, False)
            ...
            self.project.clone_scenario(name, desc)
            self.qgis_project.log_message(f"Cloned 'root' scenario. New scenario {name} addeed to the project")
        else:
